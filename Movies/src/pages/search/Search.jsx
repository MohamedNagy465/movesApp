...
export default function Search({ getTrailer = () => {} }) {
  ...
  // تهيئة AOS عند تحميل الصفحة أو تغير النتائج
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {trending.map((movie, index) => (
                <div key={movie.id} data-aos="fade-up" data-aos-delay={index * 100}>
                  <Card movie={{ ...movie, type: "movie" }} type="movie" getTrailer={getTrailer} />
                </div>
              ))}
            </div>
          </>
  ...
  return (
    <div className="p-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
      {results.map((item, index) => (
        <div key={`${item.id}-${item.type}`} data-aos="fade-up" data-aos-delay={index * 100}>
          <Card
            movie={item}
            type={item.type}
